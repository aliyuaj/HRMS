...
<div>
  ...
  WHERE personalinfo.unit='$unit'  && unitRole!='head' "));
  ...
  JOIN personalinfo ON leaveapplic.applicantID=personalinfo.staffID
  JOIN tblunit on tblunit.unitid=personalinfo.unit
  WHERE personalinfo.unit='$unit' && unitRole!='head' ORDER BY dateapplied DESC LIMIT $start,$per_page");
    ...
    if($pages_query>0){
        ...
        echo "<table  class='table table-condensed table-bordered' style='min-width:50%'>
			...
			<td><b>Ending Date</b></td><td><b>Leave Address</b></td><td><b>Status</b></td><td><b>File</b></td><td>Action</td>";
        echo "</tr></thead> ";
        $i=($per_page*($page-1))+1;
        while($result=mysqli_fetch_assoc($query5)){
            ...
            <button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">Action <span class="caret"></span></button>
                 <ul class="dropdown-menu" role="menu">
                                     <li><a data-toggle="modal" data-applic="'.$result["applicationID"].'"href="#adjust"><i class="glyphicon glyphicon-calendar"></i> Adjust date</a></li>
                    <li><a href="approve_leave.php?appID='.$result["applicationID"].'"><i class="glyphicon glyphicon-thumbs-up"></i> Approve</a></li>
                    <li><a href="deny_leave.php?appID='.$result["applicationID"].'"><i class="glyphicon glyphicon-thumbs-down"></i> Deny</a></li>
            </ul>
                </div>';
            }
				echo '</td></tr>';
...
<script>
    $(document).ready(function(){
        $('#adjust').on('show.bs.modal',function(e){
            var did=$(e.relatedTarget).data('applic');
            $(e.currentTarget).find('input[name="uid"]').val(did);
        });
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Adjust Date</h3>
            </div>
            <div class="modal-body">
                <form method="POST" action="adjustdate.php" enctype="multipart/form-data" class="form-horizontal" role="form" >
                    ...
                    <div class="form-group" id="duration">
                        ...
                        <div class="col-lg-10">
                            <input type="number" class="form-control"name="duration" min="1" placeholder="no. of days leave to be taken" required/>

                    </div>
                        </div>
                    <div class="modal-footer">
                        <input type="button" class="btn btn-default" data-dismiss="modal" value="Close"/>
                        <button type="submit" class="btn btn-primary" name="addLeave">Approve</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
