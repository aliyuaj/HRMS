...
$title = 'KASU HRMS Documents';
include  'header.php';
?>
<div>
    <div>
        <a class='btn btn-success pull-right' data-toggle='modal' href='#addDoc'><span class='glyphicon glyphicon-plus'></span> Add Document</a>
    </div>
<?php
...
if(isset($_SESSION['doc'])){
    ...
    echo '</div>';
			...
			$per_page=15;
			$pages_query=mysqli_num_rows(mysqli_query($con,"SELECT * FROM documents WHERE empID='$staffID'"));
			...
			$start=($page-1)*$per_page;
				$query5=mysqli_query($con,"SELECT * from documents WHERE empID='$staffID' LIMIT $start,$per_page");
				$count=mysqli_num_rows($query5);
				if($pages_query>0){
                    echo	"<div style='font-family:Palatino Linotype;font-size:30px;color:green;'algn='left' class='lead'>Your Documents : $pages_query</div>";
                    echo "<div class='table-responsive text-center'>";
                    echo "<table  class='table table-condensed table-bordered' style='min-width:50%'>
			<tr><thead>
			<td><b>S/No.</b></td><td ><b>File Type</b></td><td ><b>Description</b></td><td ><b>File Size</b></td><td><b>Upload Date</b></td><td><b>Action</b></td>";
            echo "</tr></thead> ";
        $i=($per_page*($page-1))+1;
        while($result=mysqli_fetch_assoc($query5)){
        echo "<tr><td align='center'>".$i."</td><td align='justify'>".$result['filetype']."</td><td align='justify'>".$result['filedesc']."</td><td align='center'>".$result['filesize']." KB"."</td><td align='center'>".$result['date']."</td>";
            echo '<td>
            <div class="btn-group">
            <button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">Action <span class="caret"></span></button>
                 <ul class="dropdown-menu" role="menu">
                    <li><a href="download.php?file_name='.$result["filename"].'"><i class="glyphicon glyphicon-download-alt"></i> Download</a></li>
                    <li><a href="#delete" data-toggle="modal" data-del="'.$result["docID"].'"><i class="glyphicon glyphicon-trash"></i> Delete</a></li>
                </ul>
                </div>
				</td></tr>';
        $i++;
