...
$title = 'KASU HRMS My Message Box';
include  'header.php';
?>
<div>
    <?php
    ...
    $per_page=15;
    $pages_query=mysqli_num_rows(mysqli_query($con,"SELECT * FROM messages WHERE recipientID='$staffID'"));
    ...
    $query5=mysqli_query($con,"SELECT * from messages WHERE recipientID='$staffID' LIMIT $start,$per_page");
    $count=mysqli_num_rows($query5);
    if($pages_query>0){
        echo	"<div style='font-family:Palatino Linotype;font-size:30px;color:green;'algn='left' class='lead'>Inbox : $pages_query</div>";
        ...
        while($result=mysqli_fetch_assoc($query5)){
            $created = date_create($result['time']);
            $created = date_format($created, 'jS F, Y | h:i:s a');
            echo "<tr><td align='center'>".$i."</td><td align='justify'>".$result['sender']."</td>
            <td align='justify'>".$result['subject']."</td><td aign='justify'>".$result['msg']."</td><td aign='justify'>".$created."</td>";
            echo '</tr>';
            $i++;
        }
        echo "</table></div>";
