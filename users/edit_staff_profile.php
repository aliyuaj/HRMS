...
if(mysqli_num_rows($squery) > 0) {
            ...
            <td colspan="2">
                <div class="form-group" id="spryselect1">
                    <select id="unit" name="unit" class="form-control">
                        ';
    if($unit!=""){
        echo '<option selected="selected" value="'.$unitID.'">'.$unit.'</option>';
    }else echo '<option value="">--Select Unit--</option>';
    ...
    echo '</select>
            ...
            <td colspan="2">
                <div class="form-group" id="spryselect1">
                    <select id="unit" name="rank" class="form-control">
                        ';
    if($rank!=""){
        echo '<option selected="selected" value="'.$rankID.'">'.$rank.'</option>';
    }else echo '<option value="">--Select Rank--</option>';
    ...
    echo '</select>
             ...
             <tr><td><span class="tl">Unit Role:</span></td>';
    echo '<td colspan="2">
        <div class="form-group">
        <select class="form-control" name="unitrole">
            <option value="Member">Member</option>
            <option value="Head">Head</option>';
    if($unitRole!=""){
        echo '<option selected="selected" value="'.$unitRole.'">'.$unitRole.'</option>';
    }
    echo '</select></div>
        </td></tr>
        <tr>
            ...
            <td colspan="2">
                <div class="form-group" id="spryselect1">
                    <select id="status" name="estatus" class="form-control">
                        ';
    if($status!=""){
        echo '<option selected="selected" value="'.$statusID.'">'.$status.'</option>';
    }else echo '<option value="">--Select Status--</option>';
    $sql = "SELECT * FROM empstatus";
    $result = mysqli_query($con,$sql);
    while($row = mysqli_fetch_array($result)) {
        $status = $row['status'];
        $statid = $row['statid'];
        echo "<option value = '".$statid."'>$status</option>";
    }
    echo '</select>
                    </div>
                </td>
            </tr>
            <tr><td><span class="tl">First Appointment:</span></td>';
    echo '<td colspan="2"><div class="form-group">
            <input type="text" class="form-control" id="datetimepicker2" value="'.$firstAppt.'" name="fappt" class="span12" placeholder="Date to birthday is Required" required/>
            </div>
        </td></tr>
...
if(isset($_POST['save'])) {
    ...
    $sunit = $_POST['unit'];
    $srank = $_POST['rank'];
    $sunitrole = $_POST['unitrole'];
    $sempstatus = $_POST['estatus'];
    $sfappt = $_POST['fappt'];
    $update = mysqli_query($con, "UPDATE personalinfo SET staffID='$sid', title='$stitle', surname='$ssurname',othernames= '$sonames',
gender='$sgender',mstatus='$smstatus',birthdate='$birthdate',gsm='$sgsm',email='$semail',contactaddress='$sresidence',bgroup='$sbgroup',nationality='$snationalty',
state='$sstate',lga='$slga',hometown='$shometown',unit='$sunit',rank='$srank',unitRole='$sunitrole',empstat='$sempstatus'
 WHERE staffID='$sid'");
    if (($update)) {
        echo $_SESSION['update'] = "<h3 style='color:green'>Edited successfully</h3>";
        header('location:edit_staff_profile.php?id='.$sid.'');
    } else {
